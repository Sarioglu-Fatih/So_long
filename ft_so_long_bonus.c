...
/*   Updated: 2022/03/25 15:22:46 by fsariogl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_command(int keycode, t_vars *vars)
{
	...
	ft_put_all_img(vars);
	res = ft_itoa(vars->move);
	mlx_string_put(vars->mlx, vars->mlx_win, 35, 75, 0x000000, "Move : ");
	mlx_string_put(vars->mlx, vars->mlx_win, 85, 75, 0x000000, res);
	free(res);
	return (0);
}
